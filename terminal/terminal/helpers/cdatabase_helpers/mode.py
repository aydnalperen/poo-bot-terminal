...
def create_mode():
    ...
    wallets_to_add = input(
        "Write the number of your wallets that you want to add this mode (put space between numbers): ")
    wallets_to_add.split()

    wallets_to_add = [wallets[int(x) - 1] for x in wallets_to_add]

    max_tax = float(input(
        "Write the maximum tax amount for this mode and press ENTER: "))
    print("\n-> Added new ModeClass")
    print("ModeClass Name:", mode_name)
    print("Wallets to buy:", wallets_to_add)

    print("Max tax amount:", max_tax)

    mode = ModeClass(mode_name, wallets_to_add, max_tax)
    mode.save_to_db()
